...
init=[0.22,0.17,-0.09,0.4];                             %initialcondition for the for the iterative solver [gamma,alpha1,alpha2,alpha3]
...
for i=1:5 % these are used to calculate derivatives for error estimation
    ...
    WL=1/(pi)*sinh(ga1a2a3(1))./(cosh(ga1a2a3(1))-cos(2*thetaV));
    funWL=sin(thetaV).*WL;
...
G=G*sqrt(mandphi(1)/pi);
IntegralGauss=trapz(thetaV,G)*2; % *2 is because only half of the distribution is calculaten in G. If the integral is something else than 1, reconsider or remove the gaussian reconstruction. 
figure(3)
